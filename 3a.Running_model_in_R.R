...
parameters <- c(R0=4, gamma= MISSING2)
...
initN <- MISSING3 # population size
initI <- MISSING4 # Infectious
initR <- MISSING5 # Immune
...
out <- ode(y = state, times = times, func = MISSING7, parms = MISSING8)
...
# Alex alternative:
plot(out2$t,out2$S,type='l',xlab="Time (d)",ylab="Number of susceptibles")
plot(out2$t,out2$I,type='l',xlab="Time (d)",ylab="Number of infectives")
plot(out2$t,out2$R,type='l',xlab="Time (d)",ylab="Number removed")
...
plot(out2$t,incidence,type='l',xlab="Time (d)",ylab="Incidence of cases")
